...
<body>
                              ...
                              <table class="center">
                                  <tr>
                                  	...
                                  	<td>
                                        ...
                                        <div class="">
                                        	<input value="<?php if (isset($request_id)) { echo $request_id; }?>" class="form-control search" name="request_id" id="request_id" style="width:300px" type="text" maxlength="200" />
                                         	<br />
                                        </div>
                                  	</td>
                                  </tr>
                                  ...
                                  <tr>
                                  	<td>
                                        ...
                                        <div class="">
                                        	<input value="<?php if (isset($rq_uuid)) { echo $rq_uuid; }?>" class="form-control search" name="rq_uuid" id="rq_uuid" style="width:300px" type="text" maxlength="200" />
                                         	<br />
                                        </div>
                                    ...
                                    <table class="table table-striped table-bordered table-hover" id="dataTables">
                                       <thead>
                                          <tr>
                                             ...
                                             <th style="font-size:12px; vertical-align:middle;">REQUEST SERVICE NAME</th>
                                             <th style="font-size:12px; vertical-align:middle;">STATUS</th>
                                             <th style="font-size:12px; vertical-align:middle;">REQUEST ID</th>
                                          </tr>
                                       </thead>
                                       <tbody>
                                    </table>
	...
	function showDetails(record_id, rq_uuid) {
		...
		$.ajax({
			url: "page/interface/online/onlineInterface.php?record_id=" + record_id,
			type: "GET",
			data: JSON.stringify({ paramName: batch }),
			dataType: "json",
			contentType: "application/json; charset=utf-8",
			success: function (data, textStatus) {
    			$("#modalRecordCreated").text(data[0].record_created);
    			$("#modalRecordUpdated").text(data[0].record_updated);
				$("#modalRQUUID").text(data[0].rq_uuid);
				$("#modalRecordType").text(data[0].record_type);
				$("#modalEventName").text(data[0].event_name);
				$("#statusModal").text(data[0].status);
	...
	function drawTable() {
		...
		$.fn.dataTable.ext.errMode = "throw";
     	mainModalTable = $("#dataTables").DataTable({
 	 		ajax: {
     			url: "page/paging/onlineInterfacePagination.php?search1=" + search1 +"&search2="
     				+ search2 + "&event_name=" + event_name + "&req_status=" + req_status + "&request_id=" + request_id
     				+ "&rq_uuid=" + rq_uuid
			},
			dom: "<lif<t>Bp>",
			language: {
            ...
            "columnDefs": [
            	{
                    "render": function ( data, type, row ) {
                        return "<button onclick=\"showDetails('" + data + "', '" + row[7] + "')\" " + "id=\"modal\" class=\"btn btn-xs btn-emp\" style=\"min-width: 0px;color:black;\">" + 
